...
const PrismAdminApp: FC = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100">
          ...
          <aside className="md:col-span-3 lg:col-span-3">
            <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden sticky top-4">
              <div className="px-4 py-3 border-b border-gray-200 dark:border-gray-700">
                ...
                <p className="text-sm text-gray-600 dark:text-gray-400">Configure roles and simulations</p>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <EditableList
          items={dayToDay}
          onChange={setDayToDay}
          title="Top 10 Day-to-Day Activities"
          icon={<ListChecks size={18} className="text-indigo-600" />}
        />
        <EditableList
          items={kras}
          onChange={setKras}
          title="Key Responsibilities (KRAs)"
          icon={<ClipboardList size={18} className="text-blue-600" />}
        />
      </div>

      {/* SKIVE Accordion + Radar */}
      <div className="grid md:grid-cols-2 gap-6">
        <div className="space-y-3">
          <AccordionItem label="Skills" defaultOpen={false}>
            {Object.entries(skiveRatings.skills).map(([subCat, obj]) => (
              <div key={subCat} className="mb-3">
                <div className="text-sm font-medium capitalize mb-2">{subCat}</div>
                <div className="space-y-3">
                  {Object.entries(obj as Record<string, number>).map(([k, v]) => (
                    <div key={k} className="">
                      <div className="flex items-baseline justify-between">
                        <label className="text-sm font-medium mr-2 capitalize">{k}</label>
                        <span className="text-xs text-gray-500 max-w-[60%] text-right">{competencyDescriptions[k] || ''}</span>
                      </div>
                      <RangeWithTicks
                        value={v}
                        onChange={(val) =>
                          setSkiveRatings((s) => ({
                            ...s,
                            skills: { ...s.skills, [subCat]: { ...(s.skills as any)[subCat], [k]: val } },
                          }))
                        }
                      />
                    </div>
          ...
          <AccordionItem label="Knowledge" defaultOpen={false}>
            {Object.entries(skiveRatings.knowledge).map(([subCat, obj]) => (
              <div key={subCat} className="mb-3">
                <div className="text-sm font-medium capitalize mb-2">{subCat}</div>
                <div className="space-y-3">
                  {Object.entries(obj as Record<string, number>).map(([k, v]) => (
                    <div key={k}>
                      ...
                      <RangeWithTicks
                        ...
                        onChange={(val) =>
                          setSkiveRatings((s) => ({
                            ...s,
                            knowledge: { ...s.knowledge, [subCat]: { ...(s.knowledge as any)[subCat], [k]: val } },
                          }))
                        }
                      />
                    </div>
          ...
          <AccordionItem label="Identity" defaultOpen={false}>
            <div className="space-y-3">
              {Object.entries(skiveRatings.identity).map(([k, v]) => (
                <div key={k}>
                  <div className="flex items-baseline justify-between">
                    <label className="text-sm font-medium mr-2 capitalize">{k}</label>
                    <span className="text-xs text-gray-500 max-w-[60%] text-right">{competencyDescriptions[k] || ''}</span>
                  ...
                  <RangeWithTicks
                    value={v as number}
                    onChange={(val) => setSkiveRatings((s) => ({ ...s, identity: { ...(s.identity as any), [k]: val } }))}
                  />
                </div>
              ))}
            </div>
          ...
          <AccordionItem label="Values" defaultOpen={false}>
            <div className="space-y-3">
              {Object.entries(skiveRatings.values).map(([k, v]) => (
                <div key={k}>
                  <div className="flex items-baseline justify-between">
                    <label className="text-sm font-medium mr-2 capitalize">{k}</label>
                    <span className="text-xs text-gray-500 max-w-[60%] text-right">{competencyDescriptions[k] || ''}</span>
                  ...
                  <RangeWithTicks
                    value={v as number}
                    onChange={(val) => setSkiveRatings((s) => ({ ...s, values: { ...(s.values as any), [k]: val } }))}
                  />
                </div>
              ))}
            </div>
          ...
          <AccordionItem label="Ethics" defaultOpen={false}>
            <div className="space-y-3">
              {Object.entries(skiveRatings.ethics).map(([k, v]) => (
                <div key={k}>
                  <div className="flex items-baseline justify-between">
                    <label className="text-sm font-medium mr-2 capitalize">{k}</label>
                    <span className="text-xs text-gray-500 max-w-[60%] text-right">{competencyDescriptions[k] || ''}</span>
                  ...
                  <RangeWithTicks
                    value={v as number}
                    onChange={(val) => setSkiveRatings((s) => ({ ...s, ethics: { ...(s.ethics as any), [k]: val } }))}
                  />
                </div>
              ))}
            </div>
        ...
        <div className="flex items-center justify-center">
          <RadarChart
            ...
            height={300}
            data={(() => {
              // Aggregate top-level means for radar: average of each category
              const avg = (vals: number[]) => (vals.length ? Math.round(vals.reduce((a, b) => a + b, 0) / vals.length) : 0);
              const skillsVals = Object.values(skiveRatings.skills).flatMap((obj: any) => Object.values(obj as any) as number[]);
              const knowledgeVals = Object.values(skiveRatings.knowledge).flatMap((obj: any) => Object.values(obj as any) as number[]);
              const identityVals = Object.values(skiveRatings.identity) as number[];
              const valuesVals = Object.values(skiveRatings.values) as number[];
              const ethicsVals = Object.values(skiveRatings.ethics) as number[];
              return [
                { subject: 'Skills', A: avg(skillsVals as number[]) },
                { subject: 'Knowledge', A: avg(knowledgeVals as number[]) },
                { subject: 'Identity', A: avg(identityVals as number[]) },
      ...
      <div className="flex justify-end">
        <button onClick={handleSaveConfig} className="flex items-center gap-2 px-5 py-2 bg-green-600 text-white rounded hover:bg-green-700">
  ...
  const StagePlaceholder = ({ title }: { title: string }) => (
    <div className="p-6">
      <h2 className="text-xl font-semibold">{title}</h2>
